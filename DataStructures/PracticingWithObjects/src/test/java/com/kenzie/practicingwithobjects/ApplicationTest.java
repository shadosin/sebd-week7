...
public class ApplicationTest {
    ...
    public void testGetVIPList_VIPFound() {
        ...
        try {
            ...
            assertEquals(2, vipList.size(), "Wrong amount of VIPs in list");
            assertEquals(expectedList, vipList, "Test: VIP ArrayList contains unexpected values");
            entryPassesByName.clear();
    ...
    @Order(6)
    @Test
    public void testGetVIPList_VIPFound_2() {
        entryPassesByName.clear();

        try {
            Constructor entryPassConstructor = EntryPass.class.getConstructor(paramTypes);
            EntryPass entryPassHomerVip = (EntryPass) entryPassConstructor.newInstance(new String[]{"Amanda","12345","vip"});
            EntryPass entryPassMaggieVip = (EntryPass) entryPassConstructor.newInstance(new String[]{"Marlena","25840","special"});
            EntryPass entryPassMargeGeneral = (EntryPass) entryPassConstructor.newInstance(new String[]{"Sarah","96301","general"});

            entryPassesByName.put("Amanda12345", entryPassHomerVip);
            entryPassesByName.put("Marlena25840", entryPassMaggieVip);
            entryPassesByName.put("Sarah96301", entryPassMargeGeneral);

            ArrayList<String> vipList = Application.getVIPList(entryPassesByName);
            ArrayList<String> expectedList = new ArrayList<>();
            expectedList.add("Amanda");
            assertEquals(1, vipList.size(), "Wrong amount of VIPs in list");
            assertEquals(expectedList, vipList, "Test: VIP ArrayList contains unexpected values");
            entryPassesByName.clear();
            assertEquals(0, Application.getVIPList(entryPassesByName).size(), "Test: No VIP returns empty list");
